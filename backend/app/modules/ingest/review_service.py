...
def save_pending(
    ...
    *,
    ...
    review_doc = safe_name(review_doc or stem)
    write_text_atomic(pending / f"{review_doc}.md", md_text)
    rec = {
        "doc": review_doc,
        "source_doc": stem,
        "kb": kb,
    ...
    if doc_id:
        rec["doc_id"] = doc_id
    write_json_atomic(review / f"{review_doc}.json", rec, indent=2)
    log.info("review_pending_saved", doc=review_doc, notsure=len(rec["notsure"]))
    return rec
...
def _write_tmp_ocr(pdf_file: Path, tmp_md: Path) -> Path | None:
    ...
    try:
        ...
        write_ocr_sidecar(pdf_file, tmp_md)
        return Path(str(tmp_md) + ".ocr.json")
    except Exception:
        log.exception("ocr_sidecar_failed_on_approve", doc=tmp_md.stem)
        return None
...
def approve(doc: str, resolutions: dict[str, str] | None = None, *, build: bool = True) -> dict:
    ...
    with INDEX_LOCK:
        ...
        if rf.exists():
            try:
                review_rec = json.loads(rf.read_text(encoding="utf-8"))
                kb = review_rec.get("kb", "default")
            except (json.JSONDecodeError, OSError):
                log.warning("review_json_unreadable", doc=doc)
        kb = safe_name(kb, default="default")
        ...
        if build and review_rec.get("kind") == "reparse" and review_rec.get("doc_id"):
            source_doc = safe_name(review_rec.get("source_doc") or doc)
            target.mkdir(parents=True, exist_ok=True)
            md_file = target / f"{source_doc}.md"
            tmp_md = (
                get_settings().data_dir
                / f"_tmp_review_reparse_{doc}_{uuid.uuid4().hex[:8]}.md"
            )
            try:
                write_text_atomic(tmp_md, resolved)
                from app.modules.ingest.tree_service import (
                    build_reparse_candidate,
                    commit_reparse_candidate,
                )

                candidate = build_reparse_candidate(
                    tmp_md,
                    doc_id=review_rec["doc_id"],
                    kb=kb,
                    final_md_path=md_file,
                )
                pdf_file = get_settings().data_dir / "pdf" / kb / f"{source_doc}.pdf"
                tmp_ocr = _write_tmp_ocr(pdf_file, tmp_md)
                result["tree"] = commit_reparse_candidate(
                    candidate,
                    resolved,
                    tmp_ocr=tmp_ocr,
                )
